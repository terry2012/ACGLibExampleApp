...
/**
 * Activity which captrures an event into the location of the Location ACG and plays it back whenever a different button is pressed
 * This should pass static validation and be caught at runtime
 */
public class IndirectCaptureEventLocationActivity extends TimingButtonLocationActivity {

    @NonNull
    @Override
    public Collection<EvilDoer> evilDoers() {
        List<EvilDoer> evilDoers = (List<EvilDoer>) super.evilDoers();
        EvilDoer evilDoer = new IndirectCaptureEventEvilDoer(R.id.location_acg_button_id, R.id.malicious_disguised_button_id, R.id.malicious_disguised_button_id_2);
        evilDoers.add(evilDoer);
        return evilDoers;
    }

    protected int contentView() {
        return R.layout.activity_location_bottom_indirect;
    }
}
